...
import 'package:grocery_delivery_side/repositories/repo_food_summary.dart';
import '../data/processResponse/api_process_response.dart';

class FoodSummaryViewModel with ChangeNotifier{
  final _foodSummaryListRepo = FoodSummaryRepository();
  ...
  Future<void> fetchFoodCodSummaryListData(IndextPageCountRequestModel data, BuildContext context) async {
    setCodSummaryListData(ApiProcessResponse.loading());
    try {
      final CodSummaryResponseModel codSummaryResponseModel = await _foodSummaryListRepo.fetchFoodCodSummaryListData(data);
  ...
  Future<void> fetchFoodWalletSummaryListData(IndextPageCountRequestModel data, BuildContext context) async {
    setWalletSummaryListData(ApiProcessResponse.loading());
    try {
      final WalletSummaryResponseModel walletSummaryListResponseModel = await _foodSummaryListRepo.fetchFoodWalletSummaryListData(data);
